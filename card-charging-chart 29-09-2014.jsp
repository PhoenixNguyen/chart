...
<style>
	.filter_row .fieldset_filter {border: solid 1px #ccc;margin-left:20px; width: auto;}
</style>
			...
			<div class="layout_1000">
				...
				<div id="w_body">
					<div class="row_sub">
						...
						<jsp:include page="card-charging-chart-search-menu.jsp" />
						...
						<div class="right_content">
							...
							<div id="filter">
								<form action="card-dashboard.html" method="get" name="filter_histogram">
									...
									<div class="filter_row">
										...
										<c:forEach var="mc" items="${paramValues.filter_merchant}">
											<c:set var="allMC" value="${allMC}${mc}," />
										</c:forEach>
										<select id="filter_merchant" name="filter_merchant" multiple="multiple" style="width:200px; padding: 6px;">
										
											<c:forEach var="merchant" items="${model.facetAllsMap['merchant'] }">
												<c:set var="merchant2" value=",${merchant.getTerm()}," />
												<option value="${merchant.getTerm() }" ${fn:contains(allMC, merchant2)?'selected':'' }>
													<c:out value="${merchant.getTerm()}"/>
												</option>
												<c:set var="listForSearchMC" value="${listForSearchMC}<option value='${merchant.getTerm()}'>${merchant.getTerm()}</option>" />
											</c:forEach>
										</select>
										...
										<%-- <label class="label_filter">Loại thẻ</label>
										<c:set var="allCt" value="," />
										<c:forEach var="ct" items="${paramValues.filter_card_type}">
											<c:set var="allCt" value="${allCt}${ct}," />
										</c:forEach>
										<select id="filter_card_type" class="select_filter" name="filter_card_type" multiple="multiple" style="width:200px; padding: 6px;">
										
											<c:forEach var="type" items="${model.facetAllsMap['type'] }">
												<c:set var="type2" value=",${type.getTerm()}," />
												<option value="${type.getTerm() }" ${fn:contains(allCt, type2)?'selected':'' }>
													<c:out value="${type.getTerm()}"/>
												</option>
												<c:set var="listForSearchCt" value="${listForSearchCt}<option value='${type.getTerm()}'>${type.getTerm()}</option>" />
											</c:forEach>
										</select> --%>
							...
							<!-- PIE CHART -->
							<h1 class="srv_title">Biểu đồ tổng quát theo trạng thái</h1>
